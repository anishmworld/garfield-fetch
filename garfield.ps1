...
if ($start.toString("yyyy-MM-dd") -gt $finish.toString("yyyy-MM-dd"))
{
	pause "The Start Date cannot be after End Date. `nPress any key to exit"
	exit
}

if ($start.toString("yyyy-MM-dd") -lt '1978-06-19' -or $finish.toString("yyyy-MM-dd") -lt '1978-06-19')
{
	pause "The earliest comic is on 19th June, 1978. `nEnter the dates on or after that. `nPress any key to exit"
	exit
}

if ($start.toString("yyyy-MM-dd") -gt $today -or $finish.toString("yyyy-MM-dd") -gt $today)
{
	pause "The date(s) is(are) in the future. `nTardis doesn't support this script... Yet. `nPress any key to exit"
	exit
}
